...
export default function BillingPage() {
  ...
  const [planId, setPlanId] = useState<"free" | "pro" | "premium">("free");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (data.session?.user?.id) {
        ...
        if (prof?.plan_id === "pro" || prof?.plan_id === "premium" || prof?.plan_id === "free") {
          setPlanId(prof.plan_id);
        }
      }
  ...
  function placeholderCheckout(plan: "pro" | "premium") {
    // Placeholder until Stripe: send to a page or external checkout link
    ...
    <AppShell>
      <main className="px-10 py-8">
        ...
        <div className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-6 text-sm text-white/70">
          ...
          <div className="mt-2">
            To actually charge money, you’ll connect Stripe, create checkout links, then update your{" "}
