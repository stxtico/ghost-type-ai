...
export default function DetectTextClient() {
  ...
  async function runDetect() {
    ...
    try {
      ...
      const aiInt = normalizeAiPercent(json);
      setAiPercent(aiInt);
      setHumanScore(
        json?.humanScore != null ? clamp(Number(json.humanScore), 0, 100) : aiInt != null ? 100 - aiInt : null
      );
      ...
      const msg =
        e?.name === "AbortError" ? "Detect request timed out. Try again." : e?.message || "Something went wrong.";
  ...
  async function saveScan() {
    ...
    const aiInt = normalizeAiPercent(result);
    if (aiInt == null) {
      setSaveMsg("Run a scan first.");
      return;
    }
    ...
    try {
      ...
      const preview = input.trim().slice(0, 260);
      ...
      const { error } = await supabase.from("scans").insert({
        ...
        // ✅ persist result + integer ai_percent (if columns exist)
        result: result,
        ai_percent: aiInt,
      });
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-sm text-white/75">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <span className={`rounded-full border px-3 py-1 text-xs ${pillFor("ai")}`}>Red: likely AI</span>
                      <span className={`rounded-full border px-3 py-1 text-xs ${pillFor("maybe")}`}>Yellow: unsure</span>
                      <span className={`rounded-full border px-3 py-1 text-xs ${pillFor("human")}`}>Green: likely human</span>
                    </div>
                  </div>
                </div>
              )}
